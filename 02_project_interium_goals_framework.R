# modified version of https://github.com/davidiles/Interim_Goals_Framework
# as part of Status of bird assessments 
...
project <- function(
    ...
    length_current_trend = 30# How many years back to calculate "current" trend over?
) {
  ...
  # generate the indicies and extract
  inds <- generate_indices(model, regions = "continent")
  indicies <- inds$samples$continent_continent
  
  # determine yeat
  year_seq <- colnames(indicies) %>% as.numeric()
  ...
  indices_StatusQuo <-  indices_StatusQuov2 <- indices_Recovery <- indices_gam <- matrix(NA, nrow = nrow(indicies), ncol = length(year_seq_projection))
  Index_baseline <- Trend_samples <- Trend_samplesv2 <- c()
  ...
  for (i in 1:nrow(indicies)){
   ...
   # i = 1
    ...
    i_dat <- data.frame(log_y = log(indicies[i,]),Year = year_seq) %>% 
...
sp_table <- rbind(sp_table,recovery_description)
...
sp_plot_index <- ggplot() +
  ...
  # Observed indices
  geom_errorbar(data = subset(indices_summarized, Year <= final_year_of_data),aes(x = Year, ymin = Index_q_0.025, ymax = Index_q_0.975), width = 0, col = "gray30")+
  geom_point(data = subset(indices_summarized, Year <= final_year_of_data),aes(x = Year, y = Index), col = "gray30")+
...
# Plot up to 2050
#coord_cartesian(ylim=c(0,max(apply(sp_projection$indicies,2,function(x) quantile(x, 0.975)))),
#                xlim=c(1970,2050))+
